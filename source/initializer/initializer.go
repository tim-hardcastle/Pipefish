...
func (iz *initializer) createStructNamesAndLabels() {
	...
	for i := range iz.ParsedDeclarations[structDeclaration] {
		...
		for i := range sT.(vm.StructType).LabelNumbers {
...
func (iz *initializer) compileFunction(node ast.Node, private bool, outerEnv *compiler.Environment, dec declarationType) *compiler.CpFunc {
	...
	case token.GOCODE:
		...
		cpF.HasGo = true
		println("In initializer", iz.cp.P.NamespacePath, body.GetToken().Source, body.GetToken().Line, "appending", body.(*ast.GolangExpression).GoFunction.Kind().String())
