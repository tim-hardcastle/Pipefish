		...
		if l.peekChar() == '.' {
			...
			if l.skipWhitespaceAfterPotentialContinuation() {
				...
				if l.afterWhitespace {
					tok = l.NewToken(token.ILLEGAL, "unmatched or misplaced continuation sign " + text.Emph(".."))
				}
				tok.Line  = l.line
				tok.ChStart = l.tstart
...
func (l *Lexer) interpretWhitespace() token.Token {
	...
	if l.ch == '.' && l.peekChar() == '.' {
		l.readChar()
		l.readChar()
		return l.NewToken(token.ILLEGAL, "a line can't begin with a continuation unless it follows a line ending with either a continuation or a comma")
	}
	...
	return l.NewToken(token.ILLEGAL, 
		fmt.Sprintf("whitespace is inconsistent with previous indentation levels %s",
		stack.ExplainWhitespaceStack(&l.whitespaceStack)))
}
...
func (l *Lexer) NewToken(tokenType token.TokenType, st string) token.Token {
	return token.Token{Type: tokenType, Literal: st, Line: l.line, ChStart: l.tstart, ChEnd: l.char}
}
