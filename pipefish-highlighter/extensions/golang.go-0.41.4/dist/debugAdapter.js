...
var Delve = class {
      ...
      if (launchArgs.request === "launch") {
        ...
        if (launchArgs.noDebug) {
          if (mode === "debug") {
            ...
            (_a = this.debugProcess.stderr) == null ? void 0 : _a.on("data", (chunk) => {
              const str = chunk.toString();
              if (this.onstderr) {
                this.onstderr(str);
              }
            });
            (_b = this.debugProcess.stdout) == null ? void 0 : _b.on("data", (chunk) => {
              const str = chunk.toString();
              if (this.onstdout) {
                this.onstdout(str);
              }
            ...
            this.debugProcess.on("close", (code) => {
              var _a2, _b2;
              if (code) {
                logError(`Process exiting with code: ${code} signal: ${(_a2 = this.debugProcess) == null ? void 0 : _a2.killed}`);
              } else {
                log(`Process exiting normally ${(_b2 = this.debugProcess) == null ? void 0 : _b2.killed}`);
              }
              if (this.onclose) {
                this.onclose(code);
              }
        ...
        dlvArgs.push("--headless=true", "--listen=" + launchArgs.host + ":" + ((_c = launchArgs.port) == null ? void 0 : _c.toString()));
        if (!this.isApiV1) {
          dlvArgs.push("--api-version=2");
        }
        if (launchArgs.showLog) {
      ...
      function connectClient(port, host, onClose) {
        setTimeout(() => {
          ...
          conn.on("connect", () => resolve2(conn)).on("error", reject).on("close", (hadError) => {
            ...
            onClose == null ? void 0 : onClose(hadError ? 1 : 0);
          });
        }, 200);
      }
      (_d = this.debugProcess.stderr) == null ? void 0 : _d.on("data", (chunk) => {
        const str = chunk.toString();
        if (this.onstderr) {
          this.onstderr(str);
        }
      });
      (_e = this.debugProcess.stdout) == null ? void 0 : _e.on("data", (chunk) => {
        const str = chunk.toString();
        if (this.onstdout) {
          this.onstdout(str);
        }
  ...
  call(command, args, callback) {
    this.connection.then(
      (conn) => {
        conn == null ? void 0 : conn.call("RPCServer." + command, args, callback);
      },
      (err) => {
        callback(err);
      }
  ...
  callPromise(command, args) {
    return new Promise((resolve2, reject) => {
      this.connection.then(
        (conn) => {
          conn == null ? void 0 : conn.call(`RPCServer.${command}`, args, (err, res) => {
  ...
  async close() {
    const forceCleanup = async () => {
      var _a;
      log(`killing debugee (pid: ${(_a = this.debugProcess) == null ? void 0 : _a.pid})...`);
...
var GoDebugSession = class extends import_vscode_debugadapter.LoggingDebugSession {
  constructor(debuggerLinesStartAt1, isServer = false, fileSystem = fs3) {
    ...
    this.packageInfo = /* @__PURE__ */ new Map();
    ...
    this.localToRemotePathMapping = /* @__PURE__ */ new Map();
    this.remoteToLocalPathMapping = /* @__PURE__ */ new Map();
  ...
  initializeRequest(response, args) {
    ...
    response.body = (_a = response.body) != null ? _a : {};
  ...
  async disconnectRequestHelper(response, args) {
    var _a, _b, _c;
    if ((_a = this.delve) == null ? void 0 : _a.delveConnectionClosed) {
      log("Skip disconnectRequestHelper as Delve's connection is already closed.");
      return;
    }
    if ((_b = this.delve) == null ? void 0 : _b.isRemoteDebugging) {
    ...
    log("Closing Delve.");
    await ((_c = this.delve) == null ? void 0 : _c.close());
  }
  ...
  inferRemotePathFromLocalPath(localPath) {
    ...
    const potentialMatchingRemoteFiles = (_a = this.remoteSourcesAndPackages.remoteSourceFilesNameGrouping.get(fileName)) != null ? _a : [];
  ...
  async toDebuggerPath(filePath) {
    var _a, _b, _c, _d;
    if (((_a = this.substitutePath) == null ? void 0 : _a.length) === 0) {
      if ((_b = this.delve) == null ? void 0 : _b.isRemoteDebugging) {
    ...
    let substituteRule;
    (_c = this.substitutePath) == null ? void 0 : _c.forEach((value) => {
    ...
    return filePath = filePath.replace(/\/|\\/g, (_d = this.remotePathSeparator) != null ? _d : "");
  }
  /**
  ...
  inferLocalPathFromRemotePath(remotePath) {
    ...
    const globSync = glob.sync(fileName, {
      matchBase: true,
      cwd: (_a = this.delve) == null ? void 0 : _a.program
    });
    const bestMatchingLocalPath = this.findPathWithBestMatchingSuffix(remotePath, globSync);
    if (bestMatchingLocalPath) {
      const fullLocalPath = path2.join((_c = (_b = this.delve) == null ? void 0 : _b.program) != null ? _c : "", bestMatchingLocalPath);
  ...
  inferLocalPathFromRemoteGoPackage(remotePath) {
    ...
    const localWorkspacePath = path2.join((_b = (_a = this.delve) == null ? void 0 : _a.program) != null ? _b : "", relativeRemotePath);
  ...
  inferLocalPathInGoPathFromRemoteGoPackage(remotePathWithLocalSeparator, relativeRemotePath) {
    ...
    const localGoPathSrcPath = path2.join(
      gopath,
      "src",
      relativeRemotePath.split((_a = this.remotePathSeparator) != null ? _a : "").join(this.localPathSeparator)
    );
  ...
  toLocalPath(pathToConvert) {
    ...
    if (!substituteRule) {
      ...
      if (gopath && indexGoModCache > 0) {
        return path2.join(
          gopath,
          pathToConvert.substr(indexGoModCache).split((_d = this.remotePathSeparator) != null ? _d : "").join(this.localPathSeparator)
        );
      }
    }
    return pathToConvert.split((_e = this.remotePathSeparator) != null ? _e : "").join(this.localPathSeparator);
  }
  async setBreakPointsRequest(response, args) {
    ...
    if (!await this.isDebuggeeRunning()) {
      ...
      (_a = this.delve) == null ? void 0 : _a.callPromise("Command", [{ name: "halt" }]).then(
        () => {
  ...
  async threadsRequest(response) {
    ...
    log("ThreadsRequest");
    (_a = this.delve) == null ? void 0 : _a.call("ListGoroutines", [], (err, out) => {
      var _a2;
      ...
      const goroutines = ((_a2 = this.delve) == null ? void 0 : _a2.isApiV1) ? out : out.Goroutines;
  ...
  async stackTraceRequest(response, args) {
    ...
    const stackTraceIn = { id: goroutineId, depth: (_a = this.delve) == null ? void 0 : _a.stackTraceDepth };
    if (!((_b = this.delve) == null ? void 0 : _b.isApiV1)) {
      Object.assign(stackTraceIn, { full: false, cfg: (_c = this.delve) == null ? void 0 : _c.loadConfig });
    }
    (_e = this.delve) == null ? void 0 : _e.call(
      ((_d = this.delve) == null ? void 0 : _d.isApiV1) ? "StacktraceGoroutine" : "Stacktrace",
      [stackTraceIn],
      async (err, out) => {
        var _a2, _b2;
        if (err) {
        ...
        const locations = ((_a2 = this.delve) == null ? void 0 : _a2.isApiV1) ? out : out.Locations;
        log("locations", locations);
        if ((_b2 = this.delve) == null ? void 0 : _b2.isRemoteDebugging) {
          await this.initializeRemotePackagesAndSources();
        }
  ...
  scopesRequest(response, args) {
    ...
    (_c = this.delve) == null ? void 0 : _c.call(
      "ListLocalVars",
      ((_a = this.delve) == null ? void 0 : _a.isApiV1) ? [listLocalVarsIn] : [{ scope: listLocalVarsIn, cfg: (_b = this.delve) == null ? void 0 : _b.loadConfig }],
      (err, out) => {
        var _a2, _b2, _c2, _d;
        if (err) {
        ...
        const locals = ((_a2 = this.delve) == null ? void 0 : _a2.isApiV1) ? out : out.Variables;
        log("locals", locals);
        this.addFullyQualifiedName(locals);
        const listLocalFunctionArgsIn = { goroutineID: goroutineId, frame: frameId };
        (_d = this.delve) == null ? void 0 : _d.call(
          "ListFunctionArgs",
          ((_b2 = this.delve) == null ? void 0 : _b2.isApiV1) ? [listLocalFunctionArgsIn] : [{ scope: listLocalFunctionArgsIn, cfg: (_c2 = this.delve) == null ? void 0 : _c2.loadConfig }],
          (listFunctionErr, outArgs) => {
            ...
            const vars = ((_a3 = this.delve) == null ? void 0 : _a3.isApiV1) ? outArgs : outArgs.Args;
            ...
            for (let i = 0; i < vars.length; ++i) {
              ...
              if (!shadowedVars.has(varName)) {
                ...
                (_b3 = shadowedVars.get(varName)) == null ? void 0 : _b3.push(i);
              }
            }
            ...
            this.getPackageInfo(this.debugState).then((packageName) => {
              ...
              (_c3 = this.delve) == null ? void 0 : _c3.call(
                "ListPackageVars",
                ((_a4 = this.delve) == null ? void 0 : _a4.isApiV1) ? [filter] : [{ filter, cfg: (_b4 = this.delve) == null ? void 0 : _b4.loadConfig }],
                (listPkgVarsErr, listPkgVarsOut) => {
                  ...
                  const globals = ((_a5 = this.delve) == null ? void 0 : _a5.isApiV1) ? listPkgVarsOut : listPkgVarsOut.Variables;
                  let initdoneIndex = -1;
    ...
    const loadChildren = async (exp, v) => {
      if (v.kind === 25 /* Struct */ && v.len > v.children.length || v.kind === 20 /* Interface */ && v.children.length > 0 && v.children[0].onlyAddr === true) {
        await this.evaluateRequestImpl({ expression: exp }).then(
          (result) => {
            var _a;
            const variable = ((_a = this.delve) == null ? void 0 : _a.isApiV1) ? result : result.Variable;
  ...
  nextRequest(response) {
    ...
    log("NextRequest");
    (_a = this.delve) == null ? void 0 : _a.call("Command", [{ name: "next" }], (err, out) => {
      var _a2;
      ...
      const state = ((_a2 = this.delve) == null ? void 0 : _a2.isApiV1) ? out : out.State;
  ...
  stepInRequest(response) {
    ...
    this.nextRequestRunning = true;
    log("StepInRequest");
    (_a = this.delve) == null ? void 0 : _a.call("Command", [{ name: "step" }], (err, out) => {
      var _a2;
      if (closureEpoch === this.continueEpoch) {
        this.nextRequestRunning = false;
      }
  ...
  stepOutRequest(response) {
    ...
    this.nextRequestRunning = true;
    log("StepOutRequest");
    (_a = this.delve) == null ? void 0 : _a.call("Command", [{ name: "stepOut" }], (err, out) => {
      var _a2;
      if (closureEpoch === this.continueEpoch) {
        this.nextRequestRunning = false;
      }
  ...
  pauseRequest(response) {
    var _a;
    log("PauseRequest");
    (_a = this.delve) == null ? void 0 : _a.call("Command", [{ name: "halt" }], (err, out) => {
      var _a2;
      if (err) {
  ...
  evaluateRequest(response, args) {
    ...
    if (!((_a = this.delve) == null ? void 0 : _a.isApiV1) && isCallCommand) {
      this.evaluateCallImpl(args).then(
        (out) => {
          var _a2, _b;
          const state = out.State;
          const returnValues = (_b = (_a2 = state == null ? void 0 : state.currentThread) == null ? void 0 : _a2.ReturnValues) != null ? _b : [];
    ...
    this.evaluateRequestImpl(args).then(
      (out) => {
        var _a2;
        const variable = ((_a2 = this.delve) == null ? void 0 : _a2.isApiV1) ? out : out.Variable;
  ...
  setVariableRequest(response, args) {
    ...
    const scope = {
      goroutineID: (_a = this.debugState) == null ? void 0 : _a.currentGoroutine.id
    };
    const setSymbolArgs = {
      Scope: scope,
      Symbol: args.name,
    ...
    (_c = this.delve) == null ? void 0 : _c.call(((_b = this.delve) == null ? void 0 : _b.isApiV1) ? "SetSymbol" : "Set", [setSymbolArgs], (err) => {
      if (err) {
  ...
  getGOROOT() {
    var _a, _b;
    if (this.delve && ((_a = this.delve) == null ? void 0 : _a.goroot)) {
      return (_b = this.delve) == null ? void 0 : _b.goroot;
    }
  ...
  initLaunchAttachRequest(response, args) {
    ...
    if (args.remotePath.length > 0) {
      ...
      const llist = localPath == null ? void 0 : localPath.split(/\/|\\/).reverse();
      ...
      if (i) {
        localPath = (llist == null ? void 0 : llist.reverse().slice(0, -i).join(this.localPathSeparator)) + this.localPathSeparator;
    ...
    if (args.substitutePath) {
      args.substitutePath.forEach((value) => {
        ...
        (_a2 = this.substitutePath) == null ? void 0 : _a2.push({
    ...
    (_b = this.delve) == null ? void 0 : _b.connection.then(
      () => {
        var _a2, _b2;
        if (!((_a2 = this.delve) == null ? void 0 : _a2.noDebug)) {
          (_b2 = this.delve) == null ? void 0 : _b2.call("GetVersion", [], (err, out) => {
            var _a3;
            if (err) {
              logError(err);
              return this.sendErrorResponse(
            ...
            const clientVersion = ((_a3 = this.delve) == null ? void 0 : _a3.isApiV1) ? 1 : 2;
            if ((out == null ? void 0 : out.APIVersion) !== clientVersion) {
              const errorMessage = `The remote server is running on delve v${out == null ? void 0 : out.APIVersion} API and the client is running v${clientVersion} API. Change the version used on the client by using the property "apiVersion" in your launch.json file.`;
  ...
  async setBreakPoints(response, args) {
    var _a, _b, _c;
    const file = normalizePath((_a = args.source.path) != null ? _a : "");
    if (!this.breakpoints.get(file)) {
      this.breakpoints.set(file, []);
    }
    const remoteFile = await this.toDebuggerPath(file);
    return Promise.all(
      (_c = (_b = this.breakpoints.get(file)) == null ? void 0 : _b.map((existingBP) => {
        var _a2, _b2;
        log("Clearing: " + existingBP.id);
        return (_b2 = this.delve) == null ? void 0 : _b2.callPromise("ClearBreakpoint", [
          ((_a2 = this.delve) == null ? void 0 : _a2.isApiV1) ? existingBP.id : { Id: existingBP.id }
        ]);
      })) != null ? _c : []
    ).then(() => {
      var _a2, _b2;
      ...
      return Promise.all(
        (_b2 = (_a2 = args.breakpoints) == null ? void 0 : _a2.map((breakpoint) => {
          var _a3, _b3, _c2, _d, _e, _f;
          if (((_b3 = (_a3 = this.delve) == null ? void 0 : _a3.remotePath) == null ? void 0 : _b3.length) === 0) {
          ...
          breakpointIn.loadArgs = (_c2 = this.delve) == null ? void 0 : _c2.loadConfig;
          breakpointIn.loadLocals = (_d = this.delve) == null ? void 0 : _d.loadConfig;
          breakpointIn.cond = breakpoint.condition;
          return (_f = this.delve) == null ? void 0 : _f.callPromise("CreateBreakpoint", [
            ((_e = this.delve) == null ? void 0 : _e.isApiV1) ? breakpointIn : { Breakpoint: breakpointIn }
            ...
            if (err.toString().startsWith("Breakpoint exists at")) {
              ...
              if (!existingBreakpoints) {
                try {
                  const listBreakpointsResponse = await ((_b4 = this.delve) == null ? void 0 : _b4.callPromise("ListBreakpoints", ((_a4 = this.delve) == null ? void 0 : _a4.isApiV1) ? [] : [{}]));
                  existingBreakpoints = ((_c3 = this.delve) == null ? void 0 : _c3.isApiV1) ? listBreakpointsResponse : listBreakpointsResponse.Breakpoints;
                } catch (error) {
              ...
              return ((_d2 = this.delve) == null ? void 0 : _d2.isApiV1) ? matchedBreakpoint : { Breakpoint: matchedBreakpoint };
            }
      ...
      if (!((_a2 = this.delve) == null ? void 0 : _a2.isApiV1)) {
      ...
      const breakpoints = convertedBreakpoints.map((bp, i) => {
        ...
        if (bp) {
          ...
          return { verified: false, line: (_a3 = args.lines) == null ? void 0 : _a3[i] };
        }
      });
  ...
  async getPackageInfo(debugState) {
    ...
    if ((_a = this.delve) == null ? void 0 : _a.isRemoteDebugging) {
      await this.initializeRemotePackagesAndSources();
    }
    const dir = path2.dirname(
      ((_c = (_b = this.delve) == null ? void 0 : _b.remotePath) == null ? void 0 : _c.length) || ((_d = this.delve) == null ? void 0 : _d.isRemoteDebugging) ? this.toLocalPath(debugState.currentThread.file) : debugState.currentThread.file
    );
    ...
    return new Promise((resolve2) => {
      ...
      (0, import_child_process.execFile)(
        ...
        { cwd: dir, env: (_a2 = this.delve) == null ? void 0 : _a2.dlvEnv },
        (err, stdout, stderr) => {
  ...
  handleReenterDebug(reason) {
    ...
    this.cleanupHandles();
    if ((_a = this.debugState) == null ? void 0 : _a.exited) {
      this.sendEvent(new import_vscode_debugadapter.TerminatedEvent());
      log("TerminatedEvent");
    } else {
      (_b = this.delve) == null ? void 0 : _b.call("ListGoroutines", [{ count: 1 }], (err, out) => {
        var _a2, _b2, _c, _d, _e;
        if (err) {
          this.logDelveError(err, "Failed to get threads");
        }
        const goroutines = ((_a2 = this.delve) == null ? void 0 : _a2.isApiV1) ? out : out.Goroutines;
        if (this.debugState && !((_b2 = this.debugState) == null ? void 0 : _b2.currentGoroutine) && goroutines.length > 0) {
          this.debugState.currentGoroutine = goroutines[0];
        }
        if (this.skipStopEventOnce) {
          log(
            `Skipping stop event for ${reason}. The current Go routines is ${(_c = this.debugState) == null ? void 0 : _c.currentGoroutine}.`
          );
          this.skipStopEventOnce = false;
          return;
        }
        if (((_d = this.overrideStopReason) == null ? void 0 : _d.length) > 0) {
          reason = this.overrideStopReason;
          this.overrideStopReason = "";
        }
        const stoppedEvent = new import_vscode_debugadapter.StoppedEvent(reason, (_e = this.debugState) == null ? void 0 : _e.currentGoroutine.id);
  ...
  async isDebuggeeRunning() {
    ...
    try {
      this.debugState = await ((_a = this.delve) == null ? void 0 : _a.getDebugState());
      return !!((_b = this.debugState) == null ? void 0 : _b.Running);
    } catch (error) {
  ...
  continue(calledWhenSettingBreakpoint) {
    ...
    const callback = (out) => {
      ...
      const state = ((_a = this.delve) == null ? void 0 : _a.isApiV1) ? out : out.State;
  ...
  evaluateCallImpl(args) {
    ...
    const returnValue = (_b = this.delve) == null ? void 0 : _b.callPromise("Command", [
      {
        name: "call",
        goroutineID: goroutineId,
        returnInfoLoadConfig: (_a = this.delve) == null ? void 0 : _a.loadConfig,
        expr: callExpr,
        unsafe: false
      }
    ]).then(
    ...
    return returnValue != null ? returnValue : Promise.resolve();
  }
  evaluateRequestImpl(args) {
    ...
    const apiV2Args = {
      ...
      Scope: scope,
      Cfg: (_a = this.delve) == null ? void 0 : _a.loadConfig
    };
    const evalSymbolArgs = ((_b = this.delve) == null ? void 0 : _b.isApiV1) ? apiV1Args : apiV2Args;
    const returnValue = (_d = this.delve) == null ? void 0 : _d.callPromise(((_c = this.delve) == null ? void 0 : _c.isApiV1) ? "EvalSymbol" : "Eval", [evalSymbolArgs]).then(
      (val) => val,
      (err) => {
        log(
  ...
  async dumpStacktrace() {
    ...
    try {
      this.debugState = await ((_a = this.delve) == null ? void 0 : _a.getDebugState());
      if (!((_b = this.debugState) == null ? void 0 : _b.currentGoroutine)) {
        goroutineId = (_d = (_c = this.debugState) == null ? void 0 : _c.currentThread.goroutineID) != null ? _d : 0;
      } else {
        goroutineId = (_f = (_e = this.debugState) == null ? void 0 : _e.currentGoroutine.id) != null ? _f : 0;
      }
    } catch (error) {
      logError("dumpStacktrace - Failed to get debugger state " + error);
    }
    const stackTraceIn = { id: goroutineId, depth: (_g = this.delve) == null ? void 0 : _g.stackTraceDepth };
    if (!((_h = this.delve) == null ? void 0 : _h.isApiV1)) {
      Object.assign(stackTraceIn, { full: false, cfg: (_i = this.delve) == null ? void 0 : _i.loadConfig });
    }
    (_k = this.delve) == null ? void 0 : _k.call(
      ((_j = this.delve) == null ? void 0 : _j.isApiV1) ? "StacktraceGoroutine" : "Stacktrace",
      [stackTraceIn],
      (err, out) => {
        var _a2;
        if (err) {
...
var _RemoteSourcesAndPackages = class extends import_events.EventEmitter {
  constructor() {
    ...
    this.remoteSourceFilesNameGrouping = /* @__PURE__ */ new Map();
  }
  /**
  ...
  async initializeRemotePackagesAndSources(delve) {
    ...
    try {
      ...
      this.remoteSourceFiles.forEach((sourceFile) => {
        ...
        (_a = this.remoteSourceFilesNameGrouping.get(fileName)) == null ? void 0 : _a.push(sourceFile);
      });
    } catch (error) {
