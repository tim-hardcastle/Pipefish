...
func (o ObjectType) String2() string {
	return string(o)
}
...
func (e *Error) Inspect(view View) string  { 
	newSource := e.Token.Source
	if newSource != "REPL input" {
		newSource = text.Emph(newSource)
	}
	if view == ViewStdOut {
		if e.Token.Line > 0 {
			return fmt.Sprintf("%s at line " + text.Yellow("%d") + " of %s", text.ERROR + e.Message, e.Token.Line, newSource)}
