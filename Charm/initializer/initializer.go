...
func New(source, input string) *Uberparser {	
	uP := &Uberparser{
...
func (uP *Uberparser) MakeParserAndTokenizedProgram(){
	...
	if !token.TokenTypeIsHeadword(tok.Type) {
		uP.MakeError(fmt.Sprintf("occurence of %v without a headword.", text.Emph(tok.Literal)), tok.Line)
		return
	}
	...
	for tok = uP.rl.NextToken(); tok.Type != token.EOF; tok = uP.rl.NextToken()  {
		if token.TokenTypeIsHeadword(tok.Type) {
			if tok.Literal == "import" {
				uP.MakeError("if it occurs " + text.Emph("import") + " must be the first headword", tok.Line)
			}
			currentSection = tokenTypeToSection[tok.Type]
			isPrivate = false
			lastTokenWasColon = false
		...
		if tok.Type == token.PRIVATE {
			if isPrivate { uP.MakeError(fmt.Sprintf(text.Emph("private") + " redeclared"), tok.Line) }
			isPrivate = true
			continue
		}
		...
		if (tok.Type == token.NEWLINE ||
			   (tok.Type == token.END && uP.rl.PeekToken().Type != token.GIVEN)) && 
		       !lastTokenWasColon && beginCount == 0 && line.Length() != 0 {
			if tok.Type == token.RPAREN {
				line.Append(tok)
			}
			switch currentSection {
			case ImportSection:
				if expressionIsAssignment {
					uP.Parser.MakeError("assignment in import section", tok.Line)
				} else {
			...
			case CmdSection:
			if expressionIsAssignment {
				uP.Parser.MakeError("assignment outside of command block in cmd section", tok.Line)
			} else {
			...
			case VarSection:
				if !expressionIsAssignment {
					uP.Parser.MakeError("function/command declaration in var section", tok.Line)
				} else {
...
func (uP *Uberparser) ParseImports() {
...
func (uP *Uberparser) ParseTypeDefs() {
		...
		for chunk := 0; chunk < len(uP.tokenizedDeclarations[typeDeclaration]); chunk++ {
			...
			if !(tok1.Type == token.IDENT && tok2.Type == token.DEF_ASSIGN) {
				uP.Parser.MakeError("malformed struct declaration", tok1.Line)
			} else {
...
func (uP *Uberparser) EvaluateTypeDefs(env *object.Environment) {
	for _, v := range(uP.parsedDeclarations[typeDeclaration]) {
		result := evaluator.Evaluate(*v, &(uP.Parser), env)
		if result.Type() == object.ERROR_OBJ {
			uP.MakeError(result.(*object.Error).Message, result.(*object.Error).Token.Line)
		}

	}
}

func (uP *Uberparser) ParseEverything() {
...
func (uP *Uberparser) InitializeEverything(env *object.Environment) {
	uP.makeFunctions()
	// Make "nil" exist.
...
func (uP *Uberparser) ImportEverything() {
	...
	for _, imp := range(uP.parsedDeclarations[importDeclaration]) {
		...
		case *ast.InfixExpression :
			if imp.TokenLiteral() != "::" {
				...
				/**/ " in the " + text.Emph("import") + " section ", imp.Token.Line)
			}
			...
			case *ast.StringLiteral :
				...
				switch lhs := lhs.(type) {
					...
					/**/ " in the " + text.Emph("import") + " section ", lhs.GetToken().Line)	
				}
			default :
			uP.MakeError("unexpected occurrence of " + text.Emph(lhs.TokenLiteral()) +
			/**/ " in the " + text.Emph("import") + " section ", lhs.GetToken().Line)	
		...
		default :
			uP.MakeError("unexpected occurrence of " + text.Emph(imp.TokenLiteral()) +
			/**/ " in the " + text.Emph("import") + " section ", imp.GetToken().Line)
		}
		code := ""
		if scriptFilepath != "" {
			dat, err := os.ReadFile(scriptFilepath)
			if err != nil {
				uP.MakeError("import " + text.Emph(scriptFilepath) + " not found", (*imp).GetToken().Line)
				return
			} else {
...
func (uP *Uberparser) returnOrderOfAssignments(declarations declarationType) *[]int {
...
func (uP *Uberparser) makeFunctions() {
	for j := functionDeclaration; j <= commandDeclaration; j++ {
		for i := 0; i < len(uP.parsedDeclarations[j]) ; i++ {
			...
			if error == nil {
				...
				if ! ok { uP.Parser.Errors = append(uP.Parser.Errors, text.Err{Msg: "too much overloading of keyword " + text.Emph(keyword), Line: -1}) }
			} else {
				uP.Parser.Errors = append(uP.Parser.Errors, text.Err{Msg: error.Message, Line: error.Token.Line})
			}
		}	
	}
}
...
func (uP *Uberparser) extractSignature(fn ast.Node) (string, signature.NamedSignature, ast.Node, ast.Node, *object.Error) {
	var (
		keyword string
...
func (uP *Uberparser) addWordsToParser(currentChunk *tokenized_code_chunk.TokenizedCodeChunk){
	inParenthesis := false
	hasPrefix := false
	hasParams := false
	...
	for j := 0; j < currentChunk.Length(); j++ {
		...
		if tok.Type != token.IDENT {
			uP.MakeError(fmt.Sprintf("inexplicable occurence of %v", tok.Literal), tok.Line)
		}
...
func (uP *Uberparser) ReportScriptFailure() {
	fmt.Print("\nThere were errors in initializing the script:\n\n")
	fmt.Println(uP.ReturnErrors())
}

func (uP *Uberparser) MakeError(msg string, line int) {
	uP.Parser.Errors = append(uP.Parser.Errors, text.Err{Msg: msg, Line: line})
}

func (uP *Uberparser) ErrorsExist() bool {
	return len(uP.Parser.Errors) > 0
}

func (uP *Uberparser) ReturnErrors() string {
	return uP.Parser.Errors.String()
}

func (uP *Uberparser) ClearErrors() {
	uP.Parser.Errors = text.Errors{}
}
