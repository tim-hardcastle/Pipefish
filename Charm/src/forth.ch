...
ex (s string):
    S = evaluate S ++ code :: tokenize(s) ++ err :: "" ++ output :: ""
...
evaluate(S state) :
    ...
    else : S ++ err :: "FORTH can't interpret " + firstInstruction
given :
    firstInstruction = S[code][0]

evalNumber(S) :
    S  ++ code :: tail(S[code]) ..
    .. ++ stack :: (S[stack] + [int S[code][0]])
...
evalBuiltin(S) :
    stackSize <  operandCount:
        ...
        S ++ (code :: tail S[code]) ..
        .. ++ (stack :: operationToApply(S[stack]))
given :
...
evalDefinition(S) :
    (len S[code]) < 3 : S ++ err :: "FORTH error: incomplete definition at end of program."
    else : S ++ (code :: (S[code] behead (len defBody) + 3)) ..
          .. ++ (defs :: (S[defs] ++ newDef))
given : 
...
given :
    extracter = func(C, R) :
        not C : S ++ err :: "FORTH error: definition unterminated by ;"
        C[0] == ";" : R
        else : this(tail(C), R + [C[0]])

evalDefCall(S) :
    evaluate(S ++ code :: S[defs][S[code][0]]) ++ code :: tail S[code]

evalOutput(S) :
    S ++ code :: tail(S[code]) ..
    .. ++ stack :: (S[stack] curtail 1) ..
    .. ++ output :: S[output] + string(S[stack][len(S[stack]) - 1]) + " "

evalEmit(S) :
    S ++ code :: tail(S[code]) ..
    .. ++ stack :: (S[stack] curtail 1) ..
    .. ++ output :: S[output] + rune(S[stack][len(S[stack]) - 1])
...
evalStringLiteral(S) :
    length == -1 :
        S ++ err :: "FORTH error: unterminated string literal"
    else : 
        S ++ code :: (S[code] behead length + 1) ..
        .. ++ output :: S[output] + lit
given:
    counter = func(C, n) :
        len(C) == 0 : -1
...
evalConditional(S) :
    ...
    type leftBranch == error:
        S ++ err :: "FORTH error: malformed conditional, left branch"
    type rightBranch == error :
        S ++ err :: "FORTH error: malformed conditional, right branch"
    S[stack][len(S[stack]) - 1] != 0 :
        (evaluate(S ++ code :: leftBranch ++ stack :: (S[stack] curtail 1))) ..
            .. ++ code :: (S[code] behead length)
    else :
        (evaluate(S ++ code :: rightBranch ++ stack :: (S[stack] curtail 1))) ..
            .. ++ code :: (S[code] behead length)
given :
...
evalDoLoop(S) :
    gatekeepStack(S, 2)
    doLength == -1 : S ++ err :: "FORTH error: malformed do loop"
    else : evalLoopBody( ..
            .. (S ++ stack :: (S[stack] curtail 2)) , doCode, 
            .. S[stack][len(S[stack]) - 1], S[stack][len(S[stack]) - 2]) ..
        .. ++ code :: (S[code] behead doLength + 1) ++ loopVar :: NIL
given :
...
evalLoopBody(S, doCode, i, upTo) :
    i >= upTo : S
    else : evalLoopBody(evaluate(S ++ loopVar :: i ++ code :: doCode), doCode, i + 1, upTo)
...
evalPushIndex(S):
    S[loopVar] == NIL : S ++ err :: "FORTH error: use of i outside of loop"
    else : S ++ code :: tail(S[code]) ++ stack :: S[stack] + [S[loopVar]]

evalVarDeclaration(S) :
    len(S[code]) < 2 : S ++ err :: "FORTH error: incomplete variable declaration"
    len(S[code]) >= 5 and type int S[code][2] == int ..
    .. and S[code][3] == "cells" and S[code][4] == "allot" :
        S ++ code :: (S[code] behead 5) ..
        .. ++ vars :: (S[vars] ++ S[code][1] :: 1000 + len(S[mem])) ..
        .. ++ mem :: S[mem] + listOfZeros(cellWidth * (1 + int S[code][2]))
    else :
        S ++ code :: (S[code] behead 2) ..
        .. ++ vars :: (S[vars] ++ S[code][1] :: 1000 + len(S[mem])) ..
        .. ++ mem :: S[mem] + [0]
given :
...
evalGetVar(S) :
    ...
    gatekeepMemory(S)
        ...
        S ++ code :: tail(S[code]) ..
        .. ++ stack :: (S[stack] curtail 1) + [S[mem][location - 1000]]
given :
...
evalPutVar(S) :
    ...
    gatekeepMemory(S)
        ...
        S ++ code :: tail(S[code]) ..
        .. ++ stack :: (S[stack] curtail 2) ..
        .. ++ mem :: (S[mem] ++ location - 1000 :: S[stack][len(S[stack]) - 2])
given :
    location = S[stack][len(S[stack]) - 1]   

evalConstDeclaration(S) :
   len(S[code]) < 2 : S ++ err :: "FORTH error: incomplete const declaration" 
   gatekeepStack(S, 1)
   else :  S ++ code :: (S[code] behead 2) ..
          .. ++ stack :: tail (S[stack]) ..
          .. ++ consts :: (S[consts] ++ S[code][1] :: S[stack][len(S[stack]) - 1])

evalConstReference(S) : 
    S ++ code :: tail(S[code]) ..
    .. ++ stack :: S[stack] + [S[consts][S[code][0]]]

gatekeepMemory(S) :
    location < 1000 :
        S ++ err :: "FORTH error: memory allocation for variables starts at 1000"
    location - 999 > len(S[mem]) :
        S ++ err :: "FORTH error: location " + string(location) + " has not been allocated"
given :
    location = S[stack][len(S[stack]) - 1]

gatekeepStack(S, n) :
    len(S[stack]) < n : S ++ err :: "FORTH error: stack underflow"
