...
func (p *Parser) FindFunction(keyword string, parameters []object.Object) (returnFunction ast.Function, returnError string) {
	if _, ok := p.FunctionTable[keyword]; !ok {
		return returnFunction, "there is no function with keyword " + text.Emph(keyword)
	}
	...
	return returnFunction, text.Emph(keyword) + " doesn't have any suitable type signature to interpret that"
}
...
func UpdateEnvironment(sig signature.NamedSignature, params []object.Object, env *object.Environment) *object.Environment { 
	for i := 0; i < len(sig); i++ {
