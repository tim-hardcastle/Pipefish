...
func (p *Parser) peekError(t token.Token) {
	p.MakeError(fmt.Sprintf("expected next token to be %s, got %s instead",
		text.Emph(string(t.Type)), text.Emph(string(p.peekToken.Type))), t)
}

func (p *Parser) noPrefixParseFnError(t token.Token) {
	p.MakeError(fmt.Sprintf("can't parse %s as a prefix.", text.Emph(string(t.Type))), t)
}

func (p *Parser) prefixSuffixError() {
	p.MakeError(fmt.Sprintf("you can't put %s before %s",text.Emph(string(p.curToken.Type)), 
		text.Emph(string(p.peekToken.Type))), p.peekToken)
}
...
func (p *Parser) ParseTokenizedChunk() *ast.Node {
	...
	if p.curToken.Type != token.EOF {
		p.MakeError(fmt.Sprintf("Charm wasn't expecting %s", text.Emph(string(p.curToken.Type))), p.curToken)
	}
	return &expn
}
...
func (p *Parser) parseExpression(precedence int) ast.Node {
	...
	if leftExp == nil {
		if p.curToken.Type == token.EOF {
			p.MakeError("Charm expected an expression before the end of the line", p.curToken)
			return nil
		}
		if p.curToken.Literal == "<-|" {
			p.MakeError("Charm expected an expression before the outdent", p.curToken)
			return nil
		}
		p.MakeError("Charm expected an expression on either side of " +
		/**/ text.Emph(p.curToken.Literal), p.curToken)
		return nil
	}
	return leftExp
}
...
func (p *Parser) parseIntegerLiteral() ast.Node {
	...
	if e != nil {
		p.MakeError(fmt.Sprintf("couldn't parse %q as integer.", p.curToken.Literal), p.curToken)
		return nil
	}
...
func (p *Parser) parseFloatLiteral() ast.Node {
	...
	if e != nil {
		p.MakeError(fmt.Sprintf("couldn't parse %q as float.", p.curToken.Literal), p.curToken)
		return nil
	}
...
func (p *Parser) parseNativePrefixExpression() ast.Node {
	...
	if expression.Right == nil {
		p.MakeError("Charm was expecting " + 
	/**/text.Emph(prefix.Literal) + " to be followed by an expression", prefix)
	}
	return expression
}
...
func (p *Parser) parseBuiltInExpression() ast.Node {
	...
	if p.curToken.Type == token.STRING {
		...
		p.MakeError("a builtin declaration should be followed by a string literal", p.curToken)
	}
	p.NextToken()
	return expression
}
...
func (p *Parser) parseFuncExpression() ast.Node {
	...
	case *ast.LazyInfixExpression :
		if root.Token.Type != token.COLON {
			p.MakeError(text.Emph("func") + " expression should have a colon", p.curToken)
			return nil
		}
		...
		return expression
	default : 
		p.MakeError("malformed " +text.Emph("func") + " expression", p.curToken)	
		return nil
	}
}
...
func (p *Parser) checkNesting() {
	...
	/**/ p.curToken.Type == token.RBRACK {
		...
		if !poppable {
			p.MakeError(describeNester(p.curToken) + " doesn't close anything", p.curToken)
			return
		}
		if !checkConsistency(popped, p.curToken) {
			p.MakeError(consistencyError(popped, p.curToken), p.curToken)
		}
	}
	if p.curToken.Type == token.EOF {
...
func consistencyError(left, right token.Token) string {
	return describeNester(left) + text.PosDescription(left) +
	/**/" unclosed by " + describeNester(right) 
}

func describeNester(t token.Token) string {
	if t.Literal == "|->" { return "indent" }
	if t.Literal == "<-|" { return "outdent" }
	if t.Type == token.EOF { return "end of line" }
	return text.Emph(t.Literal)
}
...
func (p *Parser) MakeError(msg string, tok token.Token) {
	p.Errors = append(p.Errors, &object.Error{Message: msg, Token: tok})
}
...
func (p *Parser)  ReturnErrors() string {
	...
	for _, v := range(p.Errors) {
		errorString = errorString + v.Inspect(object.ViewStdOut) + "\n"
	}
	return errorString
}
...
func (p *Parser) RecursivelySlurpSignature(node ast.Node, dflt string) signature.NamedSignature {
    ...
    case *ast.PrefixExpression :
        ...
        default :
		p.MakeError("unexpected occurrence of " + text.Emph(typednode.Operator), typednode.Token)
        }
    case *ast.InfixExpression :
        switch {
    ...
    case *ast.SuffixExpression :
		...
		p.MakeError("one of your assignments or function signatures is just too weird for Charm to handle at present", node.GetToken())
	}
    return nil
}
